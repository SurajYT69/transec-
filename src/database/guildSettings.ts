...
const logger = createLogger('database:guildSettings');
 ...
 */
export function upsertGuildSettings(settings: Partial<GuildSettings> & { guildId: string }): GuildSettings {
  try {
    const db = getDatabase();
    ...
    if (existing) {
      ...
      stmt.run(
        updated.enabled ? 1 : 0,
        updated.reactionTranslationEnabled ? 1 : 0,
        updated.allowHindi ? 1 : 0,
        updated.allowEnglish ? 1 : 0,
        updated.deleteOnReactionRemove ? 1 : 0,
        updated.maxMessageLength,
        updated.updatedAt,
        updated.guildId
      );
      return updated;
      ...
      stmt.run(
        ...
        newSettings.reactionTranslationEnabled ? 1 : 0,
        newSettings.allowHindi ? 1 : 0,
        newSettings.allowEnglish ? 1 : 0,
        newSettings.deleteOnReactionRemove ? 1 : 0,
        newSettings.maxMessageLength,
...
function getDefaultGuildSettings(guildId: string): GuildSettings {
  const now = Date.now();
  return {
    guildId,
    enabled: true,
