import { DetectedLanguage, DetectionResult, TargetLanguage } from '../types';
...
export function detectLanguage(text: string): DetectionResult {
  ...
  if (!cleanText) {
    return {
      language: DetectedLanguage.NON_TEXTUAL,
      confidence: 1.0,
      script: 'other',
      hindiSignals: 0,
      englishSignals: 0,
  ...
  if (totalScriptChars > 0 && devanagariCount / totalScriptChars > 0.3) {
    return {
      language: DetectedLanguage.HINDI_DEVANAGARI,
  ...
  let englishCount = 0;
  const classifiedWords = new Set<string>();
  ...
  let language: DetectedLanguage = DetectedLanguage.UNSUPPORTED;
  const script: 'latin' | 'devanagari' | 'mixed' | 'other' = 'latin';
  let confidence = 0.0;
 ...
 */
export function shouldTranslate(detection: DetectionResult, targetLanguage: TargetLanguage): boolean {
  if (detection.language === DetectedLanguage.NON_TEXTUAL || detection.language === DetectedLanguage.UNSUPPORTED) {
    return false;
  }
  ...
  if (targetLanguage === 'en' && detection.language === DetectedLanguage.ENGLISH) {
    return false;
  }
  if (targetLanguage === 'hi' && detection.language === DetectedLanguage.HINDI_DEVANAGARI) {
    return false;
  }
  return true;
}
